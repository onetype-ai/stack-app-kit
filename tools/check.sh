...
# Warnings never fail a 6.x run; they are printed so a reader sees them. The kit proves
# a public contract other projects build on, so it allows itself more test lines than a project.
printf '\n== warnings\n'
node --input-type=module -e 'import { Project } from "./dist/testing.js"; for (const w of Project.findWarnings({ maxTestRatio: 3 })) console.log(`warn  [${w.check}] ${w.message}`);' || true
